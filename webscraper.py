...
def mapMarket(month, year, marketNumber, marketName):
    tableContent = []
    page = requests.get(
        f'https://cdeluruguay.gob.ar/datagov/proveedoresContratadosAMRP.php?anio={year}&mes={month}&rubro={marketNumber}')
    content = page.text
...
def mapComplete():
    ...
    print('Scraping is now complete. Application will shut down')
